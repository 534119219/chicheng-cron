	...
	factory: (require) => {
		...
		const t = (key) => {
			const zh = {
				...
				groupSessionsHint: "勾选后，本次运行产生的会话会整理到侧栏「定时任务」工作区，不再堆积在“未分组”；Agent 将使用绝对路径访问任务的工作目录。",
				pushEnabled: "完成后推送通知",
				pushChannel: "推送渠道",
				...
				pushHint: "可用占位符：{name} {type} {status} {exitCode} {duration} {time} {output}（{output}=运行输出，Agent 任务的最终回答；内容留空时默认会自动附上输出）",
				...
				archiveHint: "勾选后，本次运行产生的对话（会话）会在执行成功后自动归档：从侧栏会话列表隐藏，日志仍保留，不再堆积在“未分组”。",
				back: "返回",
			};
			const en = {
				...
				groupSessionsHint: "When on, this run's conversation is filed under the “Scheduled Tasks” workspace instead of Ungrouped; the agent uses absolute paths for the work dir.",
				...
				pushHint: "Placeholders: {name} {type} {status} {exitCode} {duration} {time} {output} ({output}=run output, the agent's final answer; empty content appends it automatically)",
				...
				archiveHint: "When on, the conversation created by a successful run is archived: hidden from the sidebar session list (logs are kept), no longer piling up under Ungrouped.",
				back: "Back",
			};
...
.dshc-trigger[data-collapsed="true"]{width:36px;height:36px;padding:0;align-self:flex-start;background:transparent;border-color:transparent}
...
`;
		...
		function CronTrigger({ collapsed, onOpen }) {
			return h("button", {
				...
				title: t("trigger"),
				onClick: onOpen,
			}, iconClock(16), h("span", { className: "dshc-trigger-label" }, t("trigger")));
		}
		...
		function CronDialog({ onClose }) {
			...
			const [historyOpen, setHistoryOpen] = useState(() => typeof window !== "undefined" && window.innerWidth >= 720);
			...
			const formFields = [
				...
				h("div", { className: "dshc-field" },
					h("div", { style: { display: "flex", gap: 12, alignItems: "center" } },
						...
						draft.id !== "" && h("span", { className: "dshc-badge", "data-kind": draft.enabled ? "ok" : "idle" }, draft.enabled ? t("enabledOn") : t("disabledOff"))
					)
				),
			];
			...
			if (draft.type === "agent" || draft.type === "skill") {
				formFields.push(
					h("div", { className: "dshc-field" },
						...
						h("div", { className: "dshc-preview" }, t("groupSessionsHint")),
						draft.archiveOnSuccess && h("div", { className: "dshc-preview" }, t("archiveHint"))
					),
				);
			}
			...
			if (draft.pushEnabled) {
				formFields.push(
					...
					h("div", { className: "dshc-field" },
						...
						h("textarea", { className: "dshc-textarea", rows: 4, value: draft.pushContent, placeholder: t("pushContentPlaceholder"), onChange: (e) => patch({ pushContent: e.target.value }) })
					),
					h("div", { className: "dshc-preview" }, t("pushHint")),
				);
			}
					...
					h("div", { className: "dshc-body" },
						...
						h("div", { className: "dshc-main" },
							...
							h("div", { className: "dshc-actions" },
								...
								draft.id !== "" && h("button", { type: "button", className: "dshc-btn", onClick: toggle, disabled: saving }, draft.enabled ? t("disabledOff") : t("enabledOn")),
