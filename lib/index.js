...
async function listMessagingTargets(ctx) {
  ...
  // Fallback: the in-process service reports connected platforms even when the
  // HTTP status endpoint is unavailable (e.g. web server not mounted).
  try {
    ...
    if (messaging && typeof messaging.status === "function") {
      ...
      if (Array.isArray(platforms)) {
        ...
        if (connected.length > 0) {
          return { available: true, targets: [], connectedPlatforms: connected };
        }
      }
    }
  } catch (error) {
    console.warn("[chicheng-cron] messaging status (service) failed:", error instanceof Error ? error.message : String(error));
  }
